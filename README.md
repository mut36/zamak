...
1. 영상 유형을 고르고 `.srt` 파일을 올립니다 (키·가입 불필요)
...
app/
├── api/
│   ├── analyze/route.ts        # 파일명/자막 샘플 → 제목·연도
│   ├── enrich/route.ts         # Google Search → 감독·톤·인물 지침
│   ├── summarize/route.ts      # 영화 아닌 영상의 내용 요약
│   ├── tmdb/route.ts           # 포스터 조회 (TMDB 키)
│   └── translate/route.ts      # 청크 번역, SSE
