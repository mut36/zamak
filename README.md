...
- **키·가입 없이 바로 번역** — 서버 키로 동작합니다. 자기 Gemini API 키를 입력하면 그 키로 대신 돌릴 수도 있습니다(선택). 키는 서버에 저장되지 않고 요청 헤더로만 전달되며, 브라우저 sessionStorage에 임시 보관됩니다
...
- Gemini API 키 (서버 전용) — **필수.** 모든 요청이 기본적으로 이 키로 돌아갑니다. 사용자가 UI에서 자기 키를 입력하면 그 요청만 그 키로 대체됩니다
...
# Gemini — 모든 요청의 기본 키 (서버 전용, 클라이언트에 노출되지 않음)
...
> ⚠️ **인증 게이트 없음 (임시 상태).** 로그인/크레딧(Supabase Auth)이 아직 붙지 않아서, 지금은 **누구나 서버 키로 전체 파일을 무제한 번역**할 수 있습니다. 로그인 게이트가 배포되기 전까지 이 상태로 프로덕션에 두는 것은 비용 노출 리스크입니다 — 배포 전 로그인 게이트 완료 여부를 확인하세요.

### 티어별 청크·동시성

번역 요청의 티어는 `resolveTier()` 한 곳에서 결정됩니다. BYOK는 선택이라 **키를 입력하지 않으면 서버 키로 도는 server 티어**가 기본값이고, 자기 키를 입력하면 free 티어로 내려갑니다. 로그인/크레딧이 붙으면 이 함수 본문을 세션 조회로 교체하면 됩니다.

| | 청크 크기 | 동시성 | 근거 |
|---|---|---|---|
| free (BYOK 입력 시) | 150 | 6 | Gemini 무료 등급 RPM 15에서 유도 |
| server (기본값, 키 미입력) | 200 | 14 | **잠정값** — 배포 플랜의 동시 실행 한도 조회 후 재산정 |
...
| `GOOGLE_GENAI_API_KEY` | — | **필수.** analyze/enrich/summarize/translate 4개 라우트 전부의 기본 키. 사용자가 `x-gemini-key`를 보내면 그 요청만 대체됨 |
...
1. 영상 유형을 고르고 `.srt` 파일을 올립니다 (키·가입 불필요 — 자기 Gemini 키를 쓰고 싶다면 선택 입력)
...
app/
├── api/
│   ├── analyze/route.ts        # 파일명/자막 샘플 → 제목·연도 (서버 키, BYOK 선택)
│   ├── enrich/route.ts         # Google Search → 감독·톤·인물 지침 (서버 키, BYOK 선택)
│   ├── summarize/route.ts      # 영화 아닌 영상의 내용 요약 (서버 키, BYOK 선택)
│   ├── tmdb/route.ts           # 포스터 조회 (서버 키, BYOK 불필요)
│   └── translate/route.ts      # 청크 번역, SSE (서버 키, BYOK 선택)
...
[사용자] → UploadStep (.srt, BYOK 키는 선택)
   ...
   → useTranslation.translate
        ├── resolveTier()         BYOK 유무 → 청크 크기·동시성 (키 없으면 server)
...
Vercel Project Settings → Environment Variables에 `TMDB_API_KEY`와 `GOOGLE_GENAI_API_KEY`를 추가합니다.

> ⚠️ 위 경고 참조: 로그인 게이트 없이 이 상태로 배포하면 서버 키가 인증 없이 전체 번역에 노출됩니다.
