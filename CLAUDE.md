...
`app/lib/server/dialogueMergeService.ts`), 토막 자막 잇기
(`app/lib/joinFragments.ts`, `app/lib/server/fragmentJoinService.ts`,
판정 공통 뼈대는 `app/lib/server/judgePool.ts`), 포맷 어댑터(`app/lib/subtitles/`),
...
1. **청크 입력 블록 수 = 출력 블록 수** (재조립이 번호로 대조).
   블록 수를 바꾸는 경로는 `/polish`의 **토글 둘뿐**이다 — 짧은 대화 합치기와
   토막 자막 잇기. 둘 다 번역 파이프라인 밖이고 기본 OFF이며, 켜서 실제로
   블록이 줄면 원본 포맷 다운로드를 잃는다(`docs/decisions.md` §6-27·§6-28).
2. **타임코드는 코드가 복원** — 모델엔 번호+대사만 보내고, 모델이 뱉은 타임스탬프는 불신.
   같은 이유로 `/polish`의 줄바꿈 호출은 **글자를 못 바꾼다** — `|`와 공백을
   걷어내면 입력과 같아야 하고, 다르면 코드가 원문으로 되돌린다
   (`app/lib/verbatimGuard.ts`, `docs/decisions.md` §6-30).
