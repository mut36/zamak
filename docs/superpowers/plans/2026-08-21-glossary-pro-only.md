...
## ⚠️ Task 7 결과로 바뀐 전제 (2026-08-21)

첫 실측을 마쳤다. 요약과, 그로 인해 아래 태스크들이 어떻게 달라졌는지.

**확인된 것**
- **표기 고정은 완벽하다.** 27개 항목이 461줄·4청크에서 이형 표기 0건.
  지시문이 읽히고 작동한다는 증거다.
- **추출 입력이 `prompt=7542`** — Task 5가 E1을 유도할 때 쓴 16토큰/블록이
  실측으로 확인됐다(461블록 × 16 ≈ 7,376). 8,500블록 상한의 근거가 섰다.
- **C4·C5가 실물로 재현됐다.** 화자 셀렉트에 `로마`·`붉은 여단`·`고행대`가
  나열되고, 중복 표기 6종 때문에 React key 경고가 렌더마다 수십 줄씩 터져
  **개발 로그를 읽을 수 없을 정도다.** C5는 예상보다 심각하다.
- **§6-7의 localStorage 경고가 재현됐다.** 두 번째 시도에서 저장된 토글이
  자동으로 켜져 잘못된 작품 정보로 추출이 먼저 돌았다. Task 12에서 저장소를
  없애는 결정이 옳았다.

**바뀐 결정 — 기준표를 확정으로 격상 (커밋 `3b9799d`)**

말투 표가 틀렸고(모로 ↔ 경호대장을 양방향 존댓말로 판정), 지시문 4번 줄의
안전판이 그것을 무시해 결과물을 구했다. 그럼에도 **대표 결정으로 안전판을
뒤집었다**: 말투는 사람이 검토하고 AI는 그대로 따른다. 고쳐도 AI가 되돌리면
편집 화면이 무의미하기 때문이다. 상세는 스펙 §2-4-1.

**그래서 아래 태스크의 성격이 바뀌었다**

| 태스크 | 전 | 후 |
|---|---|---|
| Task 9 (관계 추가·구간 편집) | 있으면 좋은 것 | **이 설계의 전제.** 빠지면 "고칠 수 없는 표를 AI가 맹종"이 된다 |
| Task 8 | 준비 작업 | 여기에 **C0(말투 탭 기본 펼침)**이 추가된다 |

**C0 — 말투 표를 사람 눈앞에 내민다**: 프로를 고르면 카드가 **말투 탭이 펼쳐진
채로** 나온다. 별도 확인 버튼·강제 게이트는 두지 않는다(매번 눌러야 하는 확인은
결국 안 읽고 누르는 확인이 된다 — 대표 결정). 표기가 아니라 말투 탭이 기본인
이유는 표기는 실측에서 완벽했고 틀릴 수 있는 쪽이 말투이기 때문이다.

---

## Task 8: 편집 카드를 세 파일로 나눈다 (동작 무변경) + C0
