...
| /polish 결과의 자막 개수가 원본과 다름 | "짧은 주고받음을 한 자막으로 합치기"를 켰다면 정상 — 켠 사람에게만 §9.8의 2.5단계가 돈다. **껐는데도 다르면 버그다**: 그때는 블록을 더하거나 뺄 경로가 이 파이프라인에 하나도 없다(`spliceBlocks`는 구조적으로 개수를 보존한다) — §9.8 |
