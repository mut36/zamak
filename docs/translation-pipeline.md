...
| 1.5 | **언어 감지**(묻지 않는다) | 코드 | `detectSubtitleLanguage` (`app/lib/detectLanguage.ts`) |
| 2 | 1차 `enforceTextRules` | 코드 | `app/lib/srt.ts` |
| 2.4 | (opt-in) 토막 자막 런 수집 → AI 문장 경계 판정 → 잇기·재번호 | 코드+서버 | `app/lib/joinFragments.ts`, `/api/polish`(`task: 'join'`) → `fragmentJoinService` |
| 2.5 | (opt-in) 짧은 주고받음 후보 수집 → AI 판정 → 병합·재번호 | 코드+서버 | `app/lib/mergeDialogue.ts`, `/api/polish`(`task: 'merge'`) → `dialogueMergeService` |
...
⚠️ **이 경로의 AI는 끊을 자리만 정한다 — 글자를 바꿀 수 없다**(2026-08-26부터
코드가 강제). 돌아온 대사에서 `|`와 공백을 걷어냈을 때 보낸 것과 다르면 그 블록은
버리고 원문을 쓴다(`app/lib/verbatimGuard.ts`, 응답의 `rewrittenBlocks`가 그 수다 —
**0이 정상**). 프롬프트도 "번역하지 마"에서 "한 글자도 바꾸지 마"로 넓혔고, 화자
대시 조항은 아예 뺐다 — 대시는 코드가 붙인다(§9.7의 `speakerDashesNormalized`).
왜 필요했나는 `decisions.md` §6-30.
...
#### 언어를 묻지 않고 감지한다 (2026-08-26)

규칙 적용은 언어를 **반드시** 알아야 한다 — 한 줄 상한(한국어 18자 / 라틴계
42자)도, 문장 끝 마침표를 뗄지(한국어 `.,` / 라틴계 빈 문자열)도 언어가 정한다.
영어 자막을 한국어로 알고 돌리면 마침표가 전부 사라진다. 그런데 **그걸 사용자가
알려줄 필요는 없다**: 자막 본문 수백 줄이면 문자 체계와 기능어만으로 갈린다.

- **AI를 안 부른다.** 한글/가나/한자를 먼저 보고(거의 안 틀린다), 라틴 문자일
  때만 기능어 빈도 + 그 언어만의 글자(`ñ`·`ß`·`ç`·`à`)로 en/it/es/fr/de를 가른다.
  근거가 적거나 1·2등이 붙어 있으면 **`null`을 준다** — 찍지 않는다.
- **감지 결과를 숨기지 않는다**(불변식 5의 정신). 완료 화면이 "Italiano 자막
  규칙으로 적용했습니다"를 띄우고, 다른 언어로 다시 적용할 수 있다. 다시 적용은
  파일을 다시 읽지 않는다(`usePolish`의 `lastRun`).
- **모르는 언어면 돌리기 전에 멈춘다.** 감지는 됐는데 규칙 적용이 지원하지 않는
  언어(영어 등)면 폴백으로 밀지 않고 거절한다 — 잘못된 규칙으로 돌린 결과물이
  아무것도 안 한 것보다 나쁘다. 감지 자체가 실패하면(`null`) 한국어로 돌되
  화면이 "확신하지 못해 한국어 규칙으로 적용했습니다"라고 말한다.

**언어 축이 둘이다**(2026-08-26): `TargetLang.enabled`는 번역 도착어,
`TargetLang.polish`는 규칙 적용. 요구하는 프롬프트가 다르기 때문이다 — 번역은
`translation_rules_<code>.txt` 하나, 규칙 적용은 `line_split_`·`dialogue_merge_`·
`fragment_join_<code>.txt` 셋. 실제로 양쪽에 한쪽만 준비된 언어가 있다: 영어는
번역만, **이탈리아어는 규칙 적용만**. 서버 게이트도 갈라져 있어
(`getPolishTargetLang`) 지원하지 않는 언어는 400 `unsupported_language`로 거절한다
— 예전엔 프롬프트 파일이 없어 서비스 안쪽에서 500으로 터졌다.

#### (opt-in) 토막 자막 잇기 — 자동자막을 문장으로 (2026-08-26)

유튜브 자동자막처럼 기계가 만든 자막은 한 문장을 어절로 흩어 놓는다
(`그래서` / `내가` / `거기 갔어`). 그걸 한 문장 한 자막으로 모은다. 아래 합치기와
**별개 토글**이다 — 저건 두 화자를 `- A`/`- B`로 모으고 이건 한 화자의 잘린 한
문장을 한 줄로 잇는다. 산출물도, 묶는 개수도, 판정 질문도 다르다.

- **경계를 정하는 것은 문장이지 글자 수가 아니다.** 이 기능의 핵심 설계다.
  "줄 상한이 찰 때까지 붙인다"는 자막을 부자연스럽게 만든다 — 글자 수는 문장이
  어디서 끝나는지 모른다. 그래서 역할을 이렇게 나눈다:
  - **코드는 런만 뽑는다**(`collectFragmentRuns`): 짧은 한 줄 블록이
    `FRAGMENT_RUN_MAX_GAP_MS`(0.4초) 이내로 이어지는 구간, 최대
    `FRAGMENT_RUN_MAX_BLOCKS`(8)개. 발화 도중에 잘린 토막은 사이가 거의 없고
    문장이 끝나면 숨을 쉬며 벌어진다 — 공짜로 얻는 신호다.
  - **모델은 런 안의 문장 경계를 답한다**(`prompts/common/fragment_join_ko.txt`):
    `[런번호] 1-3, 4-5`. 자리 번호는 **런 안의 위치**이지 자막 번호가 아니다.
  - **상한은 거부권으로만 쓴다**(`applyFragmentJoins`): 묶음이 한 자막(2줄 =
    `lineMaxChars`×2)이나 `SUBTITLE_MAX_DURATION_MS`(7초)를 넘으면 **그 묶음을
    버린다**. 상한에 맞춰 잘라 붙이지 않는다.
- **줄바꿈은 여기서 안 한다.** 이은 결과는 한 줄로 나가고, 상한을 넘으면 아래
  3~7단계(초과 수집 → AI 줄바꿈 → 2줄 상한)가 의미 단위로 두 줄을 만든다.
  이 모듈은 줄바꿈을 몰라도 된다.
- **읽기 속도는 안 나빠진다**: 노출이 첫 시작~마지막 끝이라 글자 수도 시간도
  원본 합계 그대로다 — CPS가 변하지 않는다.
- 걸러내기: 모르는 런 번호, 범위 밖 자리, 길이 1 묶음, 한 런 안에서 겹치는
  묶음은 전부 버린다(`readJoinGroups`). 답이 없는 런은 자동으로 "안 이음".
- **합치기보다 먼저 돈다.** 이쪽은 망가진 입력을 고치는 일이라, 고쳐진 뒤라야
  합치기가 보는 "짧은 한 줄짜리 블록"이 진짜 짧은 대사다.

#### (opt-in) 짧은 주고받음 합치기 — **블록 수를 바꾸는 유일한 경로** (2026-08-25)

`자극제?` / `그래`처럼 짧은 대사가 두 블록으로 흩어져 있으면, 기준 문서 §I.6이
말하는 이중 화자 자막(`- A` / `- B` 한 블록)으로 모은다. 타임코드 조정과 같은
**기본 OFF의 opt-in**이지만 성격이 한 단계 세다 — 켜면 자막 개수가 줄어든다.

- **판정은 두 단계.** 코드(`collectDialogueCandidates`)가 산술로 확인되는 것만
  걸러 후보 쌍을 만들고, "정말 서로 다른 두 화자인가"는 모델이 답한다
  (`prompts/common/dialogue_merge_ko.txt` → `[번호] Y|N` 한 줄씩). 모델은 대사를
  한 글자도 못 건드린다 — 돌아오는 것은 번호 목록뿐이라, 판정이 엉뚱해도 최악은
  "안 합침"이거나 "합치면 안 될 둘을 합침"이고 텍스트는 바뀌지 않는다.
- **코드 게이트**: 각 블록이 한 줄이고 대시 두 글자를 붙여도 `lineMaxChars` 안,
  간격 ≤ `DIALOGUE_MERGE_MAX_GAP_MS`(1s), 합친 노출 ≤ `DIALOGUE_MERGE_MAX_SPAN_MS`
  (5s), 태그·가사·화면 글자·이미 대시가 있는 줄 제외, 쉼표나 `…`로 끝나는 줄 제외.
  한 블록은 한 쌍에만 들어간다.
- **완결 문장 검사는 코드가 안 한다.** §I.6은 각 줄이 완결 문장이길 요구하지만
  한국어 반말(`그래`·`가자`)은 마침표도 없고 `endsASentence`가 아는 어미도 아닌
  채로 문장을 끝낸다 — 코드가 판정하면 이 기능이 잡아야 할 사례가 통째로 빠진다.
  그래서 명시적 미완결 신호(쉼표·`…`)만 코드가 걸러 내고 나머지는 프롬프트의 N
  조항이 맡는다.
- **후보가 0건이면 모델을 안 부른다** — 상한 초과 0건일 때와 같은 약속이다.
  판정 실패(청크 예외)도 미승인으로 떨어져 원본이 그대로 남는다.
- **타임코드는 코드가 만든다**(불변식 2): 앞 블록의 시작 ~ 뒤 블록의 끝. 병합이
  한 건이라도 있으면 전체를 1부터 다시 번호 매긴다.
- **대가: 다운로드가 `.srt` 하나로 줄어든다.** 원본 포맷 되돌리기는 번역문을 원본
  큐 자리에 꽂는 방식이라(`emitInOriginalFormat`), 사라진 큐에 옛 대사가 그대로
  남는다. `buildDownloads`의 `blockCountChanged`가 그 경로를 닫고, 업로드 화면은
  토글을 켤 때 이 대가를 먼저 말한다.
- 사용량은 규칙 적용과 같은 `phase='polish'`로 남는다(같은 화면·라우트·한도).
  그 행의 `blocks`는 자막이 아니라 **후보 쌍 수**다.
...
**무제한 테스터는 그 한도에도 안 걸린다**(마이그레이션 `0018`, 2026-08-26).
`0013`이 면제한 것은 크레딧 차감뿐이었는데, 이 경로는 차감이 아니라 한도가
천장이라 무제한 계정도 하루 5회에 막혔다 — "무제한"이 지키지 못하는 약속이었다.
면제는 버킷을 안 가리고(새 버킷마다 같은 사고를 반복하지 않으려고), **호출
횟수는 계속 센다** — `api_rate_limits`에 남아야 그 계정이 얼마나 쓰는지 보인다.
...
| /polish가 "아직 규칙 적용을 지원하지 않습니다"로 거절 | 정상 — 감지된 언어에 규칙 적용 프롬프트가 없다(현재 한국어·이탈리아어만). 열려면 `languages.ts`의 그 행에 `polish: true` + 프롬프트 3종 — §9.8 |
| 언어를 잘못 인식함 | 완료 화면의 "다른 언어로 다시 적용"으로 되돌린다(파일을 다시 안 읽는다). 재발하면 `detectLanguage.ts`의 `FUNCTION_WORDS`·`SIGNATURE_CHARS`와 1.5배 마진을 본다 — §9.8 |
| /polish 결과가 **번역돼서** 나옴 | 2026-08-26에 실제로 있었던 사고다 — 이탈리아어 자막이 한국어 설정으로 돌아 파일의 70%가 한국어 지시와 함께 모델로 갔다. 지금은 두 겹으로 막혀 있다: 언어 감지(§9.8 1.5단계)와 `verbatimGuard`(글자가 바뀐 블록은 원문으로 되돌림). 그래도 나온다면 응답의 `rewrittenBlocks`와 서버 로그 `[polish] N blocks came back rewritten`을 먼저 볼 것 — `decisions.md` §6-30 |
...
| /polish에서 "오늘 사용할 수 있는 횟수를 모두 썼습니다" | 정상 — `RATE_LIMITS.polish`(하루 5회). 크레딧을 안 쓰는 라우트라 이 한도가 유일한 천장이다. **무제한 테스터 계정인데도 뜬다면** 마이그레이션 `0018_unlimited_rate_limit.sql`이 그 프로젝트에 안 돌아간 것이다(개발용·배포용 둘 다 필요) — §9.8 |
| /polish 결과의 자막 개수가 원본과 다름 | "짧은 주고받음 합치기"나 "토막 자막 잇기"를 켰다면 정상 — 켠 사람에게만 §9.8의 2.5단계가 돈다. **껐는데도 다르면 버그다**: 그때는 블록을 더하거나 뺄 경로가 이 파이프라인에 하나도 없다(`spliceBlocks`는 구조적으로 개수를 보존한다) — §9.8 |
| 자동자막이 토막인데 안 이어짐 | "토막 난 자동자막을 문장으로 잇기"를 켰는지 먼저 본다(기본 OFF). 켰는데도면 런 수집에 걸렸을 수 있다 — 간격이 0.4초를 넘거나, 블록이 두 줄이거나, 태그·가사가 섞였거나, 런이 8개에서 잘렸다. 다 통과하면 모델이 안 묶은 것이다(애매하면 안 묶는 게 기본) — §9.8 |
| 이었더니 서로 다른 두 문장이 한 자막에 붙음 | 모델의 오판이다. `fragment_join_ko.txt` 규칙 2·6(문장 끝에서 끊기·애매하면 묶지 않기)을 조인다. 재발이 잦으면 `FRAGMENT_RUN_MAX_GAP_MS`를 좁혀 런 자체를 짧게 만든다 — §9.8 |
| 이은 자막이 너무 길거나 오래 떠 있음 | 천장이 안 먹은 것이다 — `applyFragmentJoins`가 `lineMaxChars`×2와 `SUBTITLE_MAX_DURATION_MS`로 묶음을 버린다. 넘는 결과가 나오면 그 검사 회귀(`joinFragments.test.ts`) — §9.8 |
| 합칠 만한데 안 합쳐짐 | 코드 게이트에 걸렸는지 먼저 본다(두 줄짜리 블록, 태그, 1초 넘는 간격, 합쳐서 5초 초과, 대시 두 글자 붙이면 18자 초과). 다 통과하는데도 안 합쳐지면 모델이 N을 준 것이다 — 애매하면 N이 기본값이다(`dialogue_merge_ko.txt`) — §9.8 |
| 합치면 안 될 둘이 합쳐짐 | 모델의 오판이다. 프롬프트의 N 조항(`dialogue_merge_ko.txt`)을 조이거나, 재발이 잦으면 코드 게이트(간격·구간)를 좁힌다 — §9.8 |
