...
--
-- 0018_unlimited_rate_limit.sql부터는 **레이트 리밋도 면제**된다(버킷 전부).
-- 크레딧을 안 쓰는 경로(규칙 적용 등)는 차감이 아니라 한도가 천장이라, 그걸
-- 안 풀면 "무제한"이 지키지 못하는 약속이 된다. 호출 횟수는 계속 센다 —
-- api_rate_limits에 남아야 이 계정이 얼마나 쓰는지 볼 수 있다.
--
