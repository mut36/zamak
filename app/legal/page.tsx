  ...
  return (
    ...
    >
      ...
      {/*
        ...
        **가격 변경 불소급 문단(2026-08-21, §6-23)**은 인상과 같은 날 들어왔다.
        선불로 산 번역권에 유효기간이 없다는 건, 산 뒤에 가격이 오르면 "내가
        가진 장수가 깎이나"라는 질문이 생긴다는 뜻이다. 그 답을 약관이 먼저
        해두지 않으면 인상 때마다 개별 문의로 돌아온다.

        차감 단위(1,200줄당 1장, 올림)는 2026-08-21에 '파일 1개당 1장'에서
        바뀌었다(docs/decisions.md §6-22). 이용자가 얼마를 내는지 정하는
        조건이라 약관에 적어야 하고, 숫자는 `BLOCKS_PER_CREDIT`·화면 문구
        (`COPY.credits.rule`)와 같아야 한다 — 세 곳이 갈라지면 갈라진 쪽이
        분쟁에서 이용자에게 유리하게 읽힌다.
      */}
      <Section title='번역권 구매와 취소·환불' id='payment'>
        <p className='m-0'>
          ZAMAK은 <b>번역권</b>을 미리 구매해 사용하는 선불 방식입니다. 결제가
          확인되면 계정에 바로 적립되고, 번역권에는 유효기간이 없습니다.
        </p>
        <p>
          번역권 <b>1장으로 자막 1,200줄까지</b> 번역할 수 있습니다. 그보다 긴
          파일은 1,200줄마다 1장씩 더 사용하며(예: 1,874줄이면 2장), 차감되는
          장수는 번역을 시작하기 전에 화면에 표시됩니다.
        </p>
        <p>
        ...
        <p>
          <b>이미 사용한 번역권</b>은 환불되지 않습니다. 번역이 시작되면 결과물의
          제공이 개시된 것이므로, 전자상거래법 제17조 제2항 제5호에 따라 그
          번역권에 대해서는 청약철회가 제한됩니다. 번역권은 번역이 시작되는
        ...
        <p>
          <b>번역권의 가격은 변경될 수 있습니다.</b> 변경하는 경우 시행일
          이전에 홈페이지에 공지하며, 변경된 가격은 공지 이후의 구매에만
          적용됩니다. <b>이미 구매하신 번역권의 장수와 사용 조건은 가격 변경의
          영향을 받지 않습니다</b> — 유효기간이 없으므로 언제 쓰시든 그렇습니다.
        </p>
        <p>
