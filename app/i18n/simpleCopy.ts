...
export const COPY = {
  ...
  landing: {
    ...
    // (`lineMaxChars`, 한국어 25자). 프로필이 바꾸는 건 **노출 시간**뿐이므로
  ...
  credits: {
    ...
    tooLargeBody: (max: number, actual: number) =>
      `번역권 1편당 최대 ${max.toLocaleString()}줄까지 번역할 수 있습니다. 현재 파일은 ${actual.toLocaleString()}줄입니다.`,
    startOver: '다른 파일 올리기',
  },
  ...
  settings: {
    ...
    glossaryBadge: '고급',
    glossaryDesc:
      '인물명 표기를 통일하고 인물 간 존대·반말을 지정합니다. 약 20초가 추가로 소요됩니다.',
