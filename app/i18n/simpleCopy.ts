...
export const COPY = {
  ...
  polish: {
    navLink: '규칙 적용',
    title: '자막 규칙 적용',
    sub: '이미 번역된 자막을 방송 표기 규칙에 맞게 다듬어 드립니다.\n언어는 자동으로 인식합니다(한국어·이탈리아어). 번역은 하지 않고, 타임코드는 아래를 켰을 때만 손댑니다.',
    ...
    // 짧은 대화 합치기 — 타임코드 조정과 같은 **기본 OFF의 opt-in**이지만
    // 성격이 한 단계 더 세다: 켜면 자막 블록의 개수 자체가 바뀐다. 그래서
    // 문구가 "무엇이 바뀌는지"보다 **"무엇을 포기하는지"**(원본 포맷 다운로드)를
    // 먼저 말한다 — 되돌릴 수 없는 쪽을 나중에 알게 하면 안 된다.
    merge: {
      title: '짧은 주고받음을 한 자막으로 합치기',
      desc: '연달아 나오는 짧은 대화를 대시(-) 두 줄짜리 한 자막으로 모읍니다',
      note: '자막 개수가 줄어들기 때문에 결과는 .srt로만 내려받을 수 있습니다. 서로 다른 두 사람의 대화가 분명할 때만 합치고, 애매하면 그대로 둡니다.',
    },
    mergeLine: (n: number) =>
      `짧은 주고받음 ${n}쌍을 각각 한 자막으로 합쳤습니다`,
    mergeSrtOnly: '자막 개수가 바뀌어 .srt로만 내려받을 수 있습니다',

    // 토막 자막 잇기 — 합치기와 같은 대가(블록 수 변경)를 치르지만 켤 상황이
    // 다르다. 저건 방송 규칙(§I.6)을 지키는 일이고 이건 자동자막이라는 **망가진
    // 입력**을 고치는 일이라, 한 토글에 묶으면 자동자막을 안 쓰는 사람이
    // 필요 없는 판정 비용을 치른다.
    join: {
      title: '토막 난 자동자막을 문장으로 잇기',
      desc: '어절 단위로 잘린 자막을 한 문장씩 모읍니다',
      note: '유튜브 자동자막처럼 한 문장이 여러 자막으로 잘려 있을 때 켜세요. 문장이 끝나는 자리에서만 잇고, 길이를 채우려고 다음 문장을 끌어오지 않습니다. 이 기능도 자막 개수를 바꿔 결과는 .srt로만 내려받을 수 있습니다.',
    },
    joinLine: (n: number) =>
      `토막 나 있던 자막 ${n}개를 앞 문장에 이어 붙였습니다`,

    // 언어는 **묻지 않고 감지한다**. 대신 결과를 숨기지 않는다 — 줄 상한도
    // 마침표 정책도 언어가 정하므로, 잘못 짚었으면 결과물 전체가 틀린다.
    // 그래서 완료 화면이 어느 언어로 돌렸는지 말하고 바꿀 길을 준다.
    language: {
      applied: (label: string) => `${label} 자막 규칙으로 적용했습니다`,
      guessed: (label: string) =>
        `언어를 확신하지 못해 ${label} 규칙으로 적용했습니다`,
      change: '다른 언어로 다시 적용',
    },
    languageUnsupported: (label: string) =>
      `${label} 자막은 아직 규칙 적용을 지원하지 않습니다. 현재 한국어와 이탈리아어만 됩니다.`,
  },
