...
describe('COPY: 속도 문구가 한 숫자로 통일돼 있다', () => {
  ...
  it('조건 단서 없이 속도를 파는 자리가 없다', () => {
    // 조건 없는 숫자는 2장짜리(2,400줄·2웨이브) 파일에 대해 거짓이 된다.
    //
    // 단서가 가리키는 크기가 2026-08-21에 바뀌었다(§6-22): 예전에는 크레딧
    // 상한 2,000블록이 경계였고 1,601~2,000줄이 '1장인데 두 웨이브'였다.
    // 이제 1장 = 1,200줄이라 1장짜리는 예외 없이 한 웨이브고, 단서는 곧
    // "번역권 1장 분량"을 뜻한다. 그래서 note가 인용하던 내부 수치
    // '1,600블록'은 낡았다 — 사용자가 화면에서 셀 수 있는 단위인 1,200줄로
    // 바꾼다.
    //
    // 히어로·라이트 카드는 "드라마 한 편"이라는 사람 말로 단서를 달고,
    // 속도 섹션은 note가 수치로 단다. titleTop은 단서를 지고 있지 않다 —
    // 같은 섹션 안의 note가 지면 충분하고, 제목에 조건을 밀어 넣으면
    // 읽히지 않는다.
    expect(COPY.landing.hero.sub).toContain('드라마 한 편');
    expect(COPY.settings.liteDesc).toContain('드라마 한 편');
    expect(COPY.landing.speed.note).toContain('1,200줄');
  });
